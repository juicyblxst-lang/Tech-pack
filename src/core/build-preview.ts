...
export function buildTechPackPreview(spec: GarmentSpec, artworks: ArtworkSpec[] = []) {
  ...
  document.sections=document.sections.map(section=>{
    ...
    if(section.title==='CONSTRUCTION')return {...section,items:construction.map(step=>`${step.title} · ${step.operation}${step.seam?` · ${step.seam}`:''}${step.stitch?` · ${step.stitch}`:''}`)}
    if(section.title==='ARTWORK')return {...section,items:overlays.length?overlays.flatMap(a=>[`#${a.id??'ART'} · ${a.name} · ${a.technique}`,`${a.placement} · ${a.width} × ${a.height}`,`Colors: ${a.colors||'TBD'}`,...(a.notes?[`Notes: ${a.notes}`]:[]) ]):['No artwork specified']}
    return section
  })
