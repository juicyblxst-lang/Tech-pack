...
export interface PreviewOverrides { bom?:BOMItem[]; construction?:ConstructionStep[] }
...
export function buildTechPackPreview(spec: GarmentSpec, artworks: ArtworkSpec[] = [], overrides:PreviewOverrides = {}) {
  const isHoodie = spec.category === 'hoodie'
  const poms = isHoodie ? hoodiePOMs : tshirtPOMs
  const bom = overrides.bom ?? (isHoodie ? hoodieBOM : tshirtBOM)
  const construction = overrides.construction ?? (isHoodie ? hoodieConstruction : tshirtConstruction)
  ...
  const frontElements = isHoodie ? renderHoodieFlat({...geometry,measurements:spec.measurements.map(m=>({code:m.id,name:m.id,value:m.value,unit:m.unit,tolerance:m.tolerance}))}, frontArtworks) : renderTshirtFlat({...geometry,measurements:spec.measurements.map(m=>({code:m.id,name:m.id,value:m.value,unit:m.unit,tolerance:m.tolerance}))}, frontArtworks)
