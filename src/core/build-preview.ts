...
function valueFor(spec: GarmentSpec, id: string, fallback: number) {
  return spec.measurements.find(item => item.id === id)?.value ?? fallback
}
...
export function buildTechPackPreview(spec: GarmentSpec, artworks: ArtworkSpec[] = []) {
  ...
  const overlays = artworks.map(artwork => ({
    name: artwork.name,
    ...
    assetDataUrl: artwork.assetDataUrl,
  }))
  const frontArtworks = overlays.filter(a => a.placement !== 'center-back')
  const backArtworks = overlays.filter(a => a.placement === 'center-back')
  ...
  document.sections = document.sections.map(section => {
    if (section.title === 'PRODUCT OVERVIEW') {
      return {
        ...section,
        items: [
          `Style: ${spec.name}`,
          `Category: ${spec.category === 'hoodie' ? 'Hoodie' : 'T-shirt'}`,
          `Fit: ${spec.fit}`,
          `Colorway: ${spec.colorways.join(', ') || 'TBD'}`,
        ],
      }
    }
    if (section.title === 'MEASUREMENTS') {
      return {
        ...section,
        items: poms.map(p => `${p.code} · ${p.name} · ${spec.measurements.find(m => m.id === p.id)?.value ?? 'TBD'} ${p.unit} · ±${spec.measurements.find(m => m.id === p.id)?.tolerance ?? p.tolerance ?? '—'}`),
      }
    }
    if (section.title === 'BILL OF MATERIALS') {
      return {
        ...section,
        items: bom.map(item => `${item.item} · ${item.specification}${item.quantity ? ` · Qty ${item.quantity}` : ''}${item.placement ? ` · ${item.placement}` : ''}`),
      }
    }
    if (section.title === 'CONSTRUCTION') {
      return {
        ...section,
        items: construction.map(step => `${step.title} · ${step.operation}${step.seam ? ` · ${step.seam}` : ''}${step.stitch ? ` · ${step.stitch}` : ''}`),
      }
    }
    if (section.title === 'ARTWORK') {
      return {
        ...section,
        items: overlays.length
          ? overlays.flatMap(a => [`${a.name} · ${a.technique}`, `${a.placement} · ${a.width} × ${a.height}`, `Colors: ${a.colors || 'TBD'}`])
