...
export function buildTechPackPreview(spec:GarmentSpec,artworks:ArtworkSpec[]=[]){
  ...
  const overlays=artworks.map(artwork=>({name:artwork.name,technique:artwork.technique,placement:artwork.placement,width:artwork.width,height:artwork.height,colors:artwork.colors,assetDataUrl:artwork.assetDataUrl}))
  ...
  const document=createTechPackDocument(spec.category);document.styleName=spec.name;document.sections=document.sections.map(section=>section.title==='MEASUREMENTS'?{...section,items:poms.map(p=>`${p.code}: ${p.name} — ${spec.measurements.find(m=>m.id===p.id)?.value??'TBD'} ${p.unit}`)}:section.title==='ARTWORK'?{...section,items:overlays.length?overlays.flatMap(a=>[`${a.name} · ${a.technique}`,`${a.placement} · ${a.width} × ${a.height}`,a.colors]):['No artwork specified']}:section)
