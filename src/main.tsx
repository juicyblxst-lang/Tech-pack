  ...
  return <main className="app">
    ...
    <section className="workspace">
      ...
      <section className="canvas"><div className="canvas-head"><div><p className="eyebrow">02 · DESIGN WORKSPACE</p><h1>{spec.name}</h1></div><div className="tabs">{(['flat','measurements','bom','construction','artwork'] as const).map(t => <button key={t} className={tab === t ? 'selected' : ''} onClick={() => setTab(t)}>{t}</button>)}</div></div>
        ...
        {tab === 'artwork' && <div className="artwork-editor"><label>Artwork file<input type="file" accept="image/*" onChange={e => e.target.files?.[0] && importArtwork(e.target.files[0])}/></label>{artwork.assetName && <div className="asset-preview"><img src={artwork.assetDataUrl} alt={artwork.assetName}/><span>{artwork.assetName}</span></div>}<label>Artwork name<input value={artwork.name} onChange={e => editArtwork('name', e.target.value)}/></label><label>Technique<select value={artwork.technique} onChange={e => editArtwork('technique', e.target.value as ArtworkTechnique)}><option value="screen-print">Screen print</option><option value="dtg">DTG</option><option value="embroidery">Embroidery</option><option value="heat-transfer">Heat transfer</option><option value="applique">Appliqué</option><option value="label">Label</option></select></label><label>Placement<input value={artwork.placement} onChange={e => editArtwork('placement', e.target.value)}/></label><div className="two"><label>Width<input value={artwork.width} onChange={e => editArtwork('width', e.target.value)}/></label><label>Height<input value={artwork.height} onChange={e => editArtwork('height', e.target.value)}/></label></div><label>Colors<input value={artwork.colors} onChange={e => editArtwork('colors', e.target.value)}/></label><label>Notes<textarea value={artwork.notes ?? ''} onChange={e => editArtwork('notes', e.target.value)}/></label></div>}
      </section>
    </section>
