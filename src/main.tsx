...
function App() {
  ...
  const spec = useMemo(() => {
    ...
    if (styleName) base.name = styleName
    return base
  }, [category, measurements, styleName])
  const preview = useMemo(() => {
    const built = buildTechPackPreview(spec)
    return {
      ...built,
      bom: built.bom.map(item => ({ ...item, ...bomEdits[item.id] })),
      construction: built.construction.map(item => ({ ...item, ...constructionEdits[item.id] })),
    }
  }, [spec, bomEdits, constructionEdits])
  ...
  return <main className="app">
    ...
    <section className="workspace">
      <aside className="sidebar">
        ...
        <div className="switch"><button className={category === 'tshirt' ? 'active' : ''} onClick={() => { setCategory('tshirt'); setMeasurements({}); setBomEdits({}); setConstructionEdits({}) }}>T-shirt</button><button className={category === 'hoodie' ? 'active' : ''} onClick={() => { setCategory('hoodie'); setMeasurements({}); setBomEdits({}); setConstructionEdits({}) }}>Hoodie</button></div>
        <label>Style name<input value={styleName || spec.name} onChange={e => setStyleName(e.target.value)}/></label>
